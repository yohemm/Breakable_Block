    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if ("no-block-breakable".equalsIgnoreCase(command.getName())) {
            if (sender instanceof Player) {
                ...
                do {
                    lastBlock = iter.next();
                    if (lastBlock.getType()!=Material.AIR && lastBlock.getType()!=Material.STATIONARY_LAVA && lastBlock.getType()!=Material.STATIONARY_WATER)
                        break;
                    else lastBlock = null;
                }while (iter.hasNext());
                if (lastBlock == null) {
            ...
            if (sender instanceof Player) {
                ...
                if (range == null) {
                    sender.sendMessage("impossible de selection cette ranger veuillez utilser se format : \\\"min-max\" ou \"nb\"");
                    return true;
                }
                ...
                if (treasureTemp == null) {
                    sender.sendMessage("impossible de selection cette item veuillez utilser se format : \\\"type:data\"");
                    return true;
                }
                ...
                if (args.length >= 4 ) {
                    ...
                    if (blockTemp == null) {
                        sender.sendMessage("impossible de selection ce block veuillez utilser se format : \\\"type:data\"");
                        return true;
                    }
                ...
                if (!blockAfterBreak.getItemType().isBlock()) {
                    sender.sendMessage("l'id de lors de l'état \"casser\" ne correspondait pas à un block, remplacement par bedRock....");
                    blockAfterBreak = new MaterialData(Material.BEDROCK);
                }
                ...
                if (p.getInventory().getItemInMainHand() != null){
                    ...
                    if(!tool.isItem()) tool = null;
                }

                

                BlockBreakable blockBreakable = new BlockBreakable(lastBlock, coulDown, Arrays.asList(idTreasure), range, new ItemStack(tool), blockAfterBreak);
